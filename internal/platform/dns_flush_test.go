...
func TestPrintFlushResult_Success(t *testing.T) {
	...
	if !bytes.Contains([]byte(output), []byte("chrome://net-internals/#sockets")) {
		t.Errorf("应始终包含 Chrome Socket 刷新提示，实际输出: %s", output)
	}

	if !bytes.Contains([]byte(output), []byte("edge://net-internals/#dns")) {
		t.Errorf("应始终包含 Edge 浏览器缓存提示，实际输出: %s", output)
	}
	if !bytes.Contains([]byte(output), []byte("edge://net-internals/#sockets")) {
		t.Errorf("应始终包含 Edge Socket 刷新提示，实际输出: %s", output)
	}
...
func TestPrintFlushResult_Failure_Linux(t *testing.T) {
	...
	if !bytes.Contains([]byte(output), []byte("chrome://net-internals/#dns")) || !bytes.Contains([]byte(output), []byte("chrome://net-internals/#sockets")) {
		t.Errorf("应始终包含 Chrome DNS 与 Socket 刷新提示，实际输出: %s", output)
	}
}
