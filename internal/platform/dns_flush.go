...
func PrintFlushResult(result *FlushResult, osType string) {
	...
	// 始终输出浏览器缓存清除提示
	fmt.Println("💡 提示: 如仍有问题，请手动清除浏览器 DNS 缓存，并刷新连接池/Socket 复用状态")
	fmt.Println("   Chrome:  访问 chrome://net-internals/#dns 点击 \"Clear host cache\"，再访问 chrome://net-internals/#sockets 点击 \"Flush socket pools\"")
	fmt.Println("   Edge:    访问 edge://net-internals/#dns 点击 \"Clear host cache\"，再访问 edge://net-internals/#sockets 点击 \"Flush socket pools\"")
	fmt.Println("   Firefox: 访问 about:networking#dns 点击 \"清除 DNS 缓存\"；如仍异常，建议重启浏览器")
	if osType == "darwin" {
		fmt.Println("   Safari:  前往菜单 \"开发\" > \"清空缓存\"（需先在偏好设置中启用开发菜单），必要时重启 Safari")
	}
}
