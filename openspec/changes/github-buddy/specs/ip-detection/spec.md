...
### REQ-2: 多维度 IP 检测
- **HTTPS 应用层验证**（最关键）：对候选 IP 发起真正的 HTTPS 请求（含 TLS 证书校验），验证 IP 是否真正可用
- **ICMP 检测**：对候选 IP 执行 ping 操作，获取延迟和丢包率
- **TCP 443 端口检测**：验证 HTTPS 端口连通性
- **TCP 22 端口检测**：验证 SSH 端口连通性
- 四种检测并发执行，单次全量检测耗时＜10s
- ICMP 不可用时（权限限制），自动降级为 HTTPS + TCP 端口检测

### REQ-3: IP 质量评分与筛选
- 计算每个 IP 的质量评分，综合考虑：
  - **HTTPS 验证状态**（权重最高 40%，通过/不通过）
  - 平均延迟（权重 30%，目标＜50ms）
  - 丢包率（权重 15%，目标 = 0%）
  - 端口连通状态（权重 15%，443 和 22 均通为满分）
- HTTPS 验证通过的 IP 评分显著高于仅 TCP 连通的 IP
- 所有 IP 的 HTTPS 均未通过时，回退到 TCP-only 评分模式
...
- [ ] 数据源不可达时使用内置默认 IP 列表
- [ ] 同时检测 HTTPS 应用层验证、ICMP、TCP 443、TCP 22 四个维度
- [ ] HTTPS 验证通过的 IP 评分显著高于仅 TCP 连通的 IP
- [ ] ICMP 不可用时自动降级为 HTTPS + TCP 检测
- [ ] 所有 IP 的 HTTPS 均未通过时回退到 TCP-only 模式
- [ ] 单次全量 IP 检测耗时＜10s
- [ ] 筛选出的最优 IP 优先保证 HTTPS 验证通过、延迟＜50ms、丢包率=0、443/22 端口全通
- [ ] 保留至少 2 个备选 IP
